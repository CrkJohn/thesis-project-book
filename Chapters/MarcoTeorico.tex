...
Podemos denotar entonces cada paso de una \gls{rnn} (como se muestras en la \figureref{fig:rnn-classic-simple}) como la aplicación recursiva de la formula:
\begin{equation}
...
\end{equation}
Los valores de $\vb$ y $\vc$ son valores de ajuste. La función $\softmax$ en la \equationref{eq:rnn-softmax-output} normaliza los valores dados de $\vo^{(t)}$ para estar en el intervalo $[0, 1]$, esto permite tratar las salidas de la red como probabilidades y también para evitar \gls{underfitting} por valores muy grandes.
...
Otras variaciones de $\mathrm{idf}$ se encuentran en el \tableref{table:idf}, donde $n_\tkt = |\{\vtkd \in \train : \tkt \in \vtkd\}|$.
