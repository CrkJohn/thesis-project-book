...
\begin{algorithm}[H]
  ...
  \Salida{Centroides $\mC \in \R^{n \times k}$}
  
  Inicialización aleatoria de los $k$ puntos $\mC \in \R^{n \times k}$\;
  Define vector $\vs \in \Nat^{m}$ representando la asignación de centroides a cada punto de $\mX$\;
  \Repetir{Ningún centroide cambia (e.g. $\norm{\vc_{t} - \vc_{t-1}} < \epsilon$)}{
    \ParaTodo{$\vx_i \in \mX$}{
      $\vs_i \leftarrow \argmin_{j\in\Nat} \norm{\vx_i - \vc_j}$\;
    }

    \ParaTodo{$\vc_j \in \mC$}{
      $sz \leftarrow \sum_{i} [ \vs_i = j ]$\;
      $\vc_j \leftarrow \frac{1}{sz} \sum_{i} \vx_i \cdot [ \vs_i = j ]$\;
    }
  }
  
  \caption{Algoritmo de $k$-means simplificado.}
  \label{algo:k-means}
\end{algorithm}
\DecMargin{1em}
...
\begin{figure}[H]
  ...
  \decoRule
  \caption[Iteraciones del algoritmo de $k$-means]{Iteraciones del algoritmo de $k$-means, $n = 2$. Tomado de \cite{tan2005introduction}.}
