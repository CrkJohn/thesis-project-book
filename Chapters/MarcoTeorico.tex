...
\begin{equation} \label{eq:bigram-prob-computation}
  P(\tkw_n \mid \tkw_{n-1}) = \frac{C( \tkw_{n-1} \, \tkw_{n})}{ \sum_w C(\tkw_{n-1} \, \tkw_n) } =^{\ast} \frac{C( \tkw_{n-1} \, \tkw_{n})}{ C(\tkw_{n-1}) }
\end{equation}
...
\subsubsection{Retroceso e interpolación}
...
Formalizándolo, se tiene entonces que en el \newterm{retroceso}, usamos un $n$-grama si existe evidencia suficiente, de lo contrario úsese un $(n-1)$-grama, de lo contrario sígase este procedimiento hasta que sea en el peor caso un unigrama. Es decir, solo se debe realizar un \textsl{back-off} a un $n$-grama de menor orden si existe cero evidencia de un $n$-grama de mayor orden. Mientras que en \newterm{interpolación}, siempre se realiza una mezcla las estimaciones de probabilidad de todos los estimadores de los $n$-gramas.
...
\begin{equation}
  \hat{P}(\tkw_n \mid \tkw_{n-k+1}^{n-1}) = \sum_{i=1}^{k-1} \lambda_i P(\tkw_n \mid \tkw_{n-k+i}^{n-1}) + \lambda_k P(\tkw_n)
\end{equation}

De forma que $\sum_i \lambda_i = 1$. Adicionalmente, para calcular los valores de $\lambda$, se debe considerar a esta como un hiper-parámetro, de forma que es necesario de un \newterm{conjunto de validación} para el calculo.
