...
\begin{equation} \label{eq:bayes-conditional-independence}
  P(\rx|\ry=y) = \prod_{i=1}^{d} P(\rx_i|\ry=y)
\end{equation}
...
Con la suposición de independencia condicional, en vez de computar la probabilidad condicional de clases para cada combinación de $X$, solo se debe realizar para establecer la probabilidad condicional de cada $X_i$, dado $\ry$.
...
\begin{equation} \label{eq:bayes-classifier}
  P(\ry|\rx) = \frac{P(\ry) \prod_{i=1}^{d}P(X_i|\ry)}{P(\rx)}
\end{equation}
