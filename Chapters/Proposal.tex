...
Luego se realiza un conteo de todas la palabras presentes dentro del \gls{corpus} de donde se sacan las $N$ primeras palabras \todo{Como se define N? explicar por favor} para incluirlas en el \gls{bow}.
...
La manera en que se representa un texto en forma de vector $\vs$ es por medio de la sumatoria de los vectores que representan cada palabra como se representa en la \equationref{eq:bow-word-vector-sum}, recuérdese que $\ve^{(i)}$ es un vector con un $1$ en la posición $i$ y ceros en el resto de posiciones.
