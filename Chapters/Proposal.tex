...
Como parte de la propuesta se proponen \nmodels modelos para tratar diferentes necesidades los cuales estan representados en la \figureref{fig:proposal-arch}.
...
\decoRule
\caption[Arquitectura de propuesta]{Arquitectura de propuesta.\todo[inline]{cambiar por la figura del slide 23 de la presentacion de campus party}}
...
Luego se realiza un conteo de todas la palabras presentes dentro del corpus de donde se sacan las $N$ primeras palabras \todo{Como se define N? explicar por favor} para incluirlas en el \gls{bow}.

Luego de esto se generan las etiquetas\todo{etiquetas por palabra o por tweet?} (o \emph{tags} en ingles) que se toman directamente de los textos de entrenamiento, de estos también se les realiza un conteo, que servirán para la predicción de las etiquetas según el contenido del texto.\todo{Se realiza un conteo al texto de entrenamiento?}
...
\end{equation}\todo{a esta ecauacion le falta el otro lado de la ecuación, es igual a que?}
...
Este tipo de redes tiene la característica de recordar salidas y entradas pasadas de datos, de forma que decisiones futuras respecto a una clasificación pueden tener una mejor forma de basarse\todo{ajustar la redaccion de esta frase}. Las redes neuronales (\glspl{neuralnetwork}) típicamente no pueden hacer estas operaciones ya que no tienen incorporada la habilidad de recordar entradas y salidas pasadas, sino que se realiza un entrenamiento con información de entrada y de salida que se conoce con anticipación, de forma que su salida solo se concentra en su entrada inmediata de datos.
...
La tarea de etiquetado para una \gls{lstm} y una \gls{bilstm} esta representada en la \figureref{fig:lstm-arch} y la \figureref{fig:bilstm-arch}.
...
El entrenamiento de las redes \gls{bilstm} se empieza de manera similar a la descrita en la \sectionref{sec:twitter-prediction} con el procedimiento de asignar un identificador a cada \emph{token}\todo{Que es un token en este contexto? en la Seccion 6.3.1 no se mencionan tokens en ninguna parte} y a cada \emph{tag} como en la \equationref{eq:lstm-token2id} y la \equationref{eq:lstm-tag2id}, donde $N$ es el numero de tokens y $T$ el numero de tags. De manera que el conjunto de entrenamiento $\sX$ consta de tuplas $(\text{token}, \text{tag})$\todo{¿sería entonces un conjunto de tuplas (token, tag), que pueden ser: (<PAD>, tag1), (<UNK>, tag2)? No me queda claro que los tokens hagan parte del conjunto de datos de entrenamiento}, de donde se agregan dos tipos especiales de tokens\todo{Solo habrian estos 2 tipos de tokens?} \texttt{<PAD>} y \texttt{<UNK>}, que representan relleno y algo desconocido respectivamente.
...
Una adición a las ventajas dadas por estos \emph{embeddings} es la capacidad de representar textos completos por medio de obtener la representación de cada termino (palabra) $t \in d$ y posteriormente calcular el promedio $\vs$ (\equationref{eq:embeddings-sum}).
...
La primera métrica que se usaría seria el numero de ``hits''\todo{que significa un hit correcto?} correctos para algún $K$, vea la \equationref{eq:hits-k}.
