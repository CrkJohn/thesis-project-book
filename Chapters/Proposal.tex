...
\begin{equation} \label{eq:linear-regression-estimation}
  \vtheta_i = \text{arg m\'{\i}n}_{\vtheta_i} | \hat{y}(\vx, \vtheta_i) - y|
\end{equation}
...
\begin{equation} \label{eq:ovr-prediction}
  c = \text{arg max}_i \, \sigmoid(\hat{y(\vx, \vtheta_i)})
\end{equation}
...
\begin{figure}[H]
  ...
  \end{tabular}
  \decoRule
  \caption[Prediccion de hashtags con modelos lineales]{Prediccion de hashtags con modelos lineales.}
...
Las etiquetas de respuesta son \emph{Otro} (\textsc{O}) o una de estas: \emph{Persona} (\textsc{PER}), \emph{Ubicación} (\textsc{LOC}), \emph{Organización} (\textsc{ORG}) y \emph{Misceláneo} (\textsc{MISC}), donde las partes de etiqueta \textsc{B-} y \textsc{I-} corresponden al inicio de una palabra o una posición intermedia de la entidad.

\begin{table}[H]
\centering
\begin{tabular}{l|llllll} 
  \textbf{Texto}    & Ian   & Goodfellow & trabaja & para & Google & Brain \\
  \textbf{Etiqueta} & B-PER & I-PER      & O       & O    & B-ORG  & I-ORG
\end{tabular}
\\ [1em]
\decoRule
